...
const double TARGET_SPEED = 50. * 0.99; // max speed
const double TARGET_ACC = 10. * 0.7;	// max acceleration in m/s/s
const double TARGET_JERK = 10. * 0.5;   // max jerk in m/s/s/s
const double REFRESH_TIME = 0.02;		// refresh time between two car position
...
double safety_lane_change_distance_back = 8.;   // distance from back car when we want change line
double safety_lane_change_distance_front = 15.; // distance from front car when we want change line
...
vector<vector<double>> line_decide(const double &car_s, const double &car_d, const double &car_x, const double &car_y, const double &car_yaw, const double &car_speed,
								   ...
								   vector<vector<double>> sensor_fusion)
{
	...
	if (!g_target_lane)
	{
		g_target_lane = int(round((car_d + LANE_WIDTH / 2) / LANE_WIDTH));
	}
	double target_d = LANE_WIDTH * (g_target_lane - 1. / 2.);
	...
	if (g_changing_lane)
	{
		...
		target_d = LANE_WIDTH * (g_target_lane - 1. / 2.);

		// Check if we are done changing lane
		if (abs((car_d + LANE_WIDTH / 2) / LANE_WIDTH - g_target_lane) < 0.25)
		{
		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					auto generated_path = line_decide(car_s, car_d, car_x, car_y, car_yaw, car_speed, map_waypoints_s, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy, previous_path_x, previous_path_y, sensor_fusion);
